...
function getPro($) {
  try {
    ...
    $('.credit_summary_item').each(function (i, elem) {
      ...
      creditDetails[creditText].push($('a', elem).first().text().trim()); //handle the first person

      //handle rest people  until the 'See more' links
      $('a', elem)
        .nextUntil('span')
        .each(function (index2) {
          creditDetails[creditText].push($(this).text().trim());
        });
    });
...
function getYear($) {
  return {
    year: $(`#titleYear > a:nth-child(1)`).text() || 0
  };
}
...
function getEpisodeCount($) {
  try {
    let headingText = $(`.bp_heading`).text().trim();
    if (headingText == 'Episode Guide') {
      return {
        episodes: $(`.bp_sub_heading`).text().trim(),
...
function getOriginalTitle($) {
  return {
    originalTitle:
      $('.title_wrapper > .originalTitle')
        .text()
        .replace(/\s\(original title\)/g, '')
        .trim() || getTitle($).title
  };
}
...
function getStar($) {
  return function (index, element) {
    const id = $(this)
      ...
      .replace('/name/', '')
      .trim();
    const name = $(this).find('.lister-item-image a img').attr('alt').trim();
    const photoUrl = $(this)
...
function getSimilarMovieTitle($) {
  return function (index, element) {
    const id = $(this).find('a')[0].attribs.href.split('/')[2];

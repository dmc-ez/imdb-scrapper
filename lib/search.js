...
/**
 * search - search a movie name on imdb
 *
 * @param {string}   term  serach term
 * @param {number} [n=1] count of result
 *
 * @returns {Promise<Array>} the array of the serach result
 */
function searchMovie(term, n = 1) {
  return request(
    `https://www.imdb.com/find?q=${term}&s=tt&ttype=ft&ref_=fn_al_tt_ex`
  )
    .then(data => {
      const $ = cheerio.load(data);
      let result = [];
...
module.exports = { search, simpleSearch, searchMovie, searchActor };
